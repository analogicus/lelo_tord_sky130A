...
fig_iout, axs_iout = plt.subplot_mosaic([['linearity'], ['error']],
                                sharex=True, figsize=(7, 6))
fig_iout.suptitle('DAC Linearity', fontsize=16)
...
for file in files:
    ...
    y4 = df.loc[target_times_idx]['i(v.xdut.v1)']*1e6 # in uA
    ...
    axs_iout['linearity'].plot(x, y4, marker='o', linestyle='solid', color=last_color, label=labelname)
    last_iout_color = axs_iout['linearity'].lines[-1].get_color()
    ...
    y_ep_start = y4.iloc[0]
    y_ep_end   = y4.iloc[-1]
    y_ep = y_ep_start + (y_ep_end - y_ep_start) * (x - x[0]) / (x[-1] - x[0])
    
    if 'endpoints' in fits:
        axs_iout['linearity'].plot(x, y_ep, linestyle='dashed', color=last_color)

    coeffs = np.polyfit(x, y4, 1)
    a, b = coeffs
    y_fit = a*x + b

    if 'bestfit' in fits:
        axs_iout['linearity'].plot(x, y_fit, linestyle='dotted', color=last_color)

    inl_ep = yi - y_ep
    inl_fit = yi - y_fit

    if 'endpoints' in fits:
        axs_iout['error'].plot(x, inl_ep, marker='s', linestyle='dashed', color=last_color, label='INL (end-point)')
    if 'bestfit' in fits:
        axs_iout['error'].plot(x, inl_fit, marker='v', linestyle='dotted', color=last_color, label='INL (best-fit)')
    ...
    if labelname.endswith("GtKttTtVt"): # If file is typical case, print the results
        ...
        axs_iout['linearity'].text(x=0.5, y=0.01, s=f'R^2 of best fit line: {r_squared:.6f}\n(when in typical corner)', transform=axs_iout['linearity'].transAxes, fontsize=8, verticalalignment='bottom', horizontalalignment='center')
        axs_iout['error'].text(x=0.5, y=0.01, s=f'Max INL (bf): {max_inl_fit:.6f}\nRMS INL (bf): {rms_inl_fit:.6f}\n(when in typical corner)', transform=axs_iout['error'].transAxes, fontsize=8, verticalalignment='bottom', horizontalalignment='center')
...
image_path = f"./figures/tb_dac_plot_linearity_{view}_{'_'.join(args)}"
