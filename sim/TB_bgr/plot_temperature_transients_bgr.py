...
for file in files:
    ...
    axs[1].plot(df["time"], df["v(v1b)"]-df["v(v2b)"], label="v1b-v2b", linestyle="solid")
    axs[1].plot(df["time"], df["v(v1a)"]-df["v(v2a)"], label="v1a-v2a", linestyle="solid")
    ...
    fig.tight_layout()
    fig.savefig(f"figures/{file.split('/')[-1].split('.out')[0]}_bgr.png", dpi=300, bbox_inches="tight")
